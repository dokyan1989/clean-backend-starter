...
describe('Hero Entity', function () {
  ...
  it('description will be set default to empty string if one is not passed', function (done) {
    // arrange
    const noDescriptionHero = makeFakeHero({ description: undefined });

    // act
    const createdHero = makeHero(noDescriptionHero);

    // assert
    expect(createdHero.getDescription()).to.be.empty;

    done();
  });

  it('must have attribute that is one of the following: strength, agility, intelligence', function (done) {
    // arrange
    const hero = makeFakeHero({ attribute: 'invalid' });

    // act and arrange
    expect(() => makeHero(hero)).to.throw('Hero must have attribute that is one of the following: strength, agility, intelligence.');
  ...
  it('will throw error if invalid createdAt is passed', function (done) {
    // arrange
    const heroWithInvalidCreatedAt = makeFakeHero({ createdAt: 'invalid' });

    // act and assert
    expect(() => makeHero(heroWithInvalidCreatedAt)).to.throw('Hero must have createdAt now in UTC.');
  ...
  it('will throw error if invalid updatedAt is passed', function (done) {
    // arrange
    const heroWithInvalidCreatedAt = makeFakeHero({ updatedAt: 'invalid' });

    // act and assert
    expect(() => makeHero(heroWithInvalidCreatedAt)).to.throw('Hero must have updatedAt now in UTC.');
